...
def nba_player_rebounds_stats():
    # Authenticate with Twitter API
    client = tweepy.Client(bearer_token, api_key, api_secret, access_token, access_token_secret)
    auth = tweepy.OAuth1UserHandler(api_key, api_secret, access_token, access_token_secret)
    api = tweepy.API(auth)

    # OddsAPI NBA Games API endpoint and player rebounds stats API
    ...
    nba_player_rebounds_stats_url = "https://sheet.best/api/sheets/6ffde7e5-71ac-4827-b162-18ba2d90ecb9/tabs/NBA_Player_Props_Rebounds"
    ...
    # Function to get NBA Player rebounds stats
    def get_nba_player_rebounds_stats(team_name):
        response = requests.get(nba_player_rebounds_stats_url)
        nba_player_rebounds_stats_data = response.json()

        for nba_player_rebounds_data in nba_player_rebounds_stats_data:
            if nba_player_rebounds_data["team"].lower() == team_name.lower():
                return nba_player_rebounds_data

        return None


    # Loop through first 3 upcoming games and Loop through NBA Player rebounds stats data and match the team playing with player on that team and display their stats
