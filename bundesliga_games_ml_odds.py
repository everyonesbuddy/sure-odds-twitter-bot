...
def call_bundesliga_games_ml_odds():
  # Authenticate with Twitter API
  client = tweepy.Client(bearer_token, api_key, api_secret, access_token, access_token_secret)
  auth = tweepy.OAuth1UserHandler(api_key, api_secret, access_token, access_token_secret)
  api = tweepy.API(auth)

  # OddsAPI BundesLiga Games API endpoint
  bundesliga_games_and_odds_api_url = "https://api.the-odds-api.com/v4/sports/soccer_germany_bundesliga/odds/?apiKey=5e7c521ab26381b068424419c586233a&regions=us&markets=h2h&oddsFormat=american&bookmakers=fanduel"

  # Make a request to the API
  response = requests.get(bundesliga_games_and_odds_api_url)
...
print("Script Bundesliga game ml odds running")

# Set the Eastern Time (EST) timezone
est = pytz.timezone('US/Eastern')

# Schedule the script to run every day at 2 PM EST
schedule.every().day.at("14:00").do(call_bundesliga_games_ml_odds).timezone = est
