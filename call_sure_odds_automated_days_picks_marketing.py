...
def call_sure_odds_automated_days_picks_marketing():
    ...
    try:
        ...
        filtered_data = [
            ...
            if game['betResult'] is None and game['gameCommenceTime'].startswith(today)
        ]
       ...
       # Format the gameCommenceTime to a more readable format in EST
        ...
        if selected_game['pickType'] == 'money line':
            message = (
                f"Today's Money Line Pick from {username_display}:\n"
                f"League: {league}\n"
            ...
            message = (
                f"Today's Props Pick from {username_display}:\n"
                f"League: {league}\n"
...
# call_sure_odds_automated_days_picks_marketing()
...
schedule.every(2).hours.do(call_sure_odds_automated_days_picks_marketing)
