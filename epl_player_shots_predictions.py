...
def epl_player_shots_predictions():
    ...
    for game in odds_data[:3]:
        ...
        if away_epl_players_shots_stats:
            # Call AI API with your team and player data and then recieve response back to be posted
            ai_api_url = 'https://streamfling-be.herokuapp.com/'
            ai_prompt = f"You are a soccer handicapper who posts picks on Twitter. I will provide you with some data points which include a team playing, commence time, and historical data on a player from one of the teams playing. You will use the data points and make over or under picks/predictions on the player prop projections based on their season average. The only data you should include in your posts are the pick/prediction, the teams playing, the player's last 3 games average, and the player's season average.  Do Not include any data in your post that I did not provide you, like do not include odds on your player props projections in your post cause we do not have that data. Also, do not include full sentences in your post. The post format should be short phrases and bullet points. Here are the data points: {home_team} vs {away_team}, {bookmaker_title} Odds - {home_team}: {home_team_price}, {away_team}, Draw: {draw_price}: {away_team_price}, Commence Time: {formatted_commence_time}. Player ({away_epl_players_shots_stats['player']}) Shots: Last Game: {away_epl_players_shots_stats['shotsLastGame']}, 2 Games Ago: {away_epl_players_shots_stats['shotsTwoGamesAgo']}, 3 Games Ago: {away_epl_players_shots_stats['shotsThreeGamesAgo']}, 4 Games Ago: {away_epl_players_shots_stats['shotsFourGamesAgo']}, 5 Games Ago: {away_epl_players_shots_stats['shotsFiveGamesAgo']}, Avg Last 3 Games: {away_epl_players_shots_stats['avergeLastThree']}, Szn Avg: {away_epl_players_shots_stats['shotsPerGameCurrentSeason']}."
            ai_response = requests.post(ai_api_url, json={'prompt': ai_prompt}, headers={'Content-Type': 'application/json'})
...
schedule.every().day.at("11:00").do(epl_player_shots_predictions).timezone = est
